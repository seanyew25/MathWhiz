<template>
  <div class="game-container">
      ...
      >
        ...
        <div v-if="showAdditionTable" class="addition-table-overlay">
          <div class="addition-grid-header">
            <span v-for="col in 10" :key="'col-' + col" class="column-number">{{
              col
            }}</span>
          </div>
          <div
    ...
    <button
      @click="checkAnswer"
      :disabled="gameOver"
      class="nes-btn is-primary submit-button"
    >
      Submit Answer
    </button>
...
export default {
  setup() {
    ...
    const handleHover = () => {
      if (currentQuestion.value.operator === "-") {
        ...
        currentQuestion.value.leftItems.forEach((item, i) => {
          if (
            i >=
            currentQuestion.value.leftItems.length -
        ...
        const totalEmojis =
          currentQuestion.value.leftNumber + currentQuestion.value.rightNumber;
    ...
    const endGame = () => {
      ...
      updateCurrency(this.db, "users", this.auth.currentUser.uid, money.value);
    };
    ...
    onMounted(() => {
      ...
      db.value = dbInstance;
      auth.value = authObj;
      getCurrency(db, "users", auth.currentUser.uid);
