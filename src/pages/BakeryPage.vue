<template>
  <div :class="{ 'math-game': showChat, 'wrapper': showChat, 'phone-cutscene': showPhone }">
    <div class="container-lg px-lg-0">
      ...
      <div v-if="showDecline" class="d-flex justify-content-center " style="display: flex;
align-items: center;
  height: 100vh;
  font-size:30px">
        <div class="nes-container is-rounded is-centered with-title tw-max-w-3xl"
      ...
      <div class="nes-container is-rounded with-title mt-5" style="
        background-color: rgba(255, 245, 205, 1);
        width: 90%;
        padding: 10px;
      ">
        <p class="title math-game" style="background-color: rgba(255, 245, 205, 1);margin: -2rem auto 1rem;">
          Fractions
        </p>
        <!-- Game Content -->
        <div v-if="showChat" class="row justify-content-center">
          <div class="col-11 text-center math-game">
            <TimerBar ref="timerBar" :initial-time="5" :is-running="isTimerRunning"
        ...
        <transition name="fade">
          <div class="row justify-content-center">
            <div class="col-lg-11">
              <div v-if='currentQuestion > 5' class="bonus-round text-center math-game" style="align-items: center;">
                <i class="nes-icon trophy is-small"></i>
                <span style="font-size: 15px;">Bonus Round! x2 coins enabled!</span>
                <i class="nes-icon trophy is-small"></i>
              </div>
            </div>
          </div>
        ...
        <div class="row d-flex justify-content-center">
          ...
          <div v-if="showChat" class="col-12 col-lg-6 m-0 d-flex justify-content-center">
            <div class="chat-container justify-content-center">
              <div class="nes-container is-rounded text-wrap pinned-message math-game">{{ chatText }}</div>
              ...
              <div class="cakes-grid">
                <div v-for="(row, rowIndex) in 3" :key="`row-${rowIndex}`" class="shelf-row"
                  ...
                  <div v-for="(cake, colIndex) in cakesMatrix[rowIndex]" :key="`cake-${rowIndex}-${colIndex}`"
                    class="cake-slot">
                    <img :src="cake.image" :alt="cake.type" class="cake" style="width: 100px;height:100px;">
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="showChat" class="row justify-content-center math-game">
          <div class="col-12 coins-earned text-center" style="font-size: 20px;padding:0">
            <p style="padding: 0px;">Question {{ currentQuestion }}/{{ totalQuestions }} |
              <i ref="earnedCoinIcon" class="nes-icon coin is-small"
                style="transform: scale(2);margin-bottom: 12px;"></i> Coins earned: <span
          ...
          <dialog class="nes-dialog math-game" id="instructions-dialog">
            <form method="dialog">
              ...
              <p style="text-align: center;font-size: small;">
                ...
                You have <strong>5</strong> seconds for
                each question. Good luck!
              </p>
              <menu class="dialog-menu center-button">
                <button class="nes-btn is-primary" style="text-align: center" @click="startGame">
                  Start Game
                </button>
              </menu>
            </form>
          ...
          <dialog class="nes-dialog math-game" id="finish-dialog">
            <form method="dialog">
              ...
              <menu class="dialog-menu center-button">
                ...
                <button class="nes-btn is-success" style="text-align: center" @click="restartGame">
                  Restart Game
                </button>
              </menu>
            </form>
...
export default {
  ...
  data() {
    return {
      ...
      showCakes: false,
      chatText: '',
      fullMessage: "Hello ah, I want to have a party but I only want vanilla cakes. What fraction of the total cakes are vanilla?",
      messageCaps: {
        small: 2,
        large: 3
      },
  ...
  methods: {
    ...
    async checkAnswer() {
      if (parseInt(this.numerator) === this.correctFraction.numerator && parseInt(this.denominator) === this.correctFraction.denominator) {
        ...
        await this.addMessage("Yay! I will take that amount. Let's move to the next question!", 'correct');
        await setTimeout(() => {
    ...
    animateCoinEarning(count) {
      const counterRect = this.$el.querySelector('.scrollable-messages').getBoundingClientRect();
    ...
    finishGame() {
      this.addMessage("Congratulations! You've completed all questions. Thank you for helping with the cat party!", 'correct');
      setTimeout(() => {
        this.$router.push("/game");
      }, 3000);
    },
